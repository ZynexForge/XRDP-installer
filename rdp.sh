...
create_linux_user() {
    ...
    if ! id "$username" >/dev/null 2>&1; then
        useradd -m -s /bin/bash -G sudo "$username" >/dev/null 2>&1 || die "Failed to create user"
...
EOF
    ...
    echo "$username:$password" | chpasswd >/dev/null 2>&1 || die "Failed to set password"
}

install_xrdp() {
    if ! systemctl is-active --quiet xrdp; then
        apt-get update -qq >/dev/null 2>&1
        DEBIAN_FRONTEND=noninteractive apt-get install -y -qq xrdp xorgxrdp xfce4 xfce4-goodies xfce4-terminal >/dev/null 2>&1 || die "Failed to install XRDP"
...
setup_tunnel_systemd() {
    if ! id "$TUNNEL_USER" >/dev/null 2>&1; then
        useradd -r -m -d "$TUNNEL_DIR" -s /bin/bash "$TUNNEL_USER" >/dev/null 2>&1
    fi
    
    mkdir -p "$TUNNEL_DIR/.ssh"
    chown -R "$TUNNEL_USER:$TUNNEL_USER" "$TUNNEL_DIR" >/dev/null 2>&1
    
    if [ ! -f "$TUNNEL_DIR/.ssh/id_rsa" ]; then
        ssh-keygen -t rsa -b 4096 -f "$TUNNEL_DIR/.ssh/id_rsa" -N "" -q >/dev/null 2>&1
        chmod 700 "$TUNNEL_DIR/.ssh" >/dev/null 2>&1
        chmod 600 "$TUNNEL_DIR/.ssh/id_rsa" >/dev/null 2>&1
    fi
